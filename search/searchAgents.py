...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    nodes = [node] + unvisited
    
    def edgeCost(p1, p2):
        return mazeDistance(p1, p2, problem.sGS)
    
    start = nodes[0]
    visited = set([start])
    edges = []
    for node in nodes[1:]:
        heapq.heappush(edges, (edgeCost(start, node), node))
        ...
        if nextNode not in visited:
            ...
            for node in nodes:
                if node not in visited:
                    heapq.heappush(edges, (edgeCost(nextNode, node), node))
    
    totalDistance = mstCost
